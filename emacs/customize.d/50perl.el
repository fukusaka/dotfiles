...
(defun my-cperl-mode-init ()
  ...
  (declare-function cperl-set-style "cperl-mode")

  ;; PerlStyle (perldoc perlstyle ベースの設定)
  (cperl-set-style "PerlStyle")
  (defvar cperl-indent-level 4                         "インデント幅を４にする" )
  (defvar cperl-continued-statement-offset 4           "連続する文のオフセット" )
  (defvar cperl-brace-offset 0                         "ブレースのオフセット" )
  (defvar cperl-close-paren-offset -4                  "閉じる括弧のオフセット" )
  (defvar cperl-label-offset -4                        "labelのオフセット" )
  (defvar cperl-indent-parens-as-block t               "括弧もブロックとしてインデント" )
  (defvar cperl-tab-always-indent t                    "TABをインデントにする" )
  (defvar cperl-highlight-variables-indiscriminately t "スカラを常にハイライト" )

  ;; see https://www.emacswiki.org/emacs/IndentingPerl
  ...
  ;;        ;; this hack is for continued statements inside parentheses
  ...
  (if (fboundp 'perl-completion-mode)
      (perl-completion-mode t))

  (auto-complete-mode t)
  (set (make-local-variable 'ac-sources) '(ac-source-perl-completion))
  )
...
(defun set-perl5lib-list ()
  ...
  (let* ((current-perl5lib-list (split-string (or (getenv "PERL5LIB") "") ":"))
         (path-list (cl-remove-if (lambda (x) (member x current-perl5lib-list))
                               (perllib-guess (buffer-file-name)))))
    (defvar perl5lib-list path-list)))

(defun set-perl5site ()
  "Set path into PERL5LIB from site.pl"
  (interactive)
  (set (make-local-variable 'perl5lib-list) nil)
  (set (make-local-variable 'perl5-command) "perl")
  (set-perl5lib-list))

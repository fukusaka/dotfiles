...
(defun moi::add-to-load-path (dir-path)
  (when (file-directory-p dir-path)
    (let (files dir subdirs)
      (setq files (directory-files dir-path nil "^[^\\.]"))
      (while files
        (setq dir (car files))
        (setq files (cdr files))
        (if (and (file-directory-p (concat dir-path dir))
                 (not (string-match "^x?emacs[0-9]+" dir)))
            (push dir subdirs)))

      (mapc
       '(lambda (subdir)
          (setq dir (expand-file-name (concat dir-path subdir)))
          (push dir load-path))
       (cons "." subdirs)))))

(defun moi::startup ()
  (moi::add-to-load-path moi::elisp-path)
